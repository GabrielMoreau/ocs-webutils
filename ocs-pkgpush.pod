...
Clean Python cache (for example playwright code).
...
With Secret Storage, it is possible to avoid entering your password each time you use it,
even when the session cookie has expired.
However, security then depends on your keyring.
...
In the general settings of the KeePassXC application,
you must also enable KeePassXC integration with Freedesktop.org's C<Secret Service>.
At the same time, enable SSH agent integration (if you also want to use KeePassXC for SSH).

Since KeePassXC does not allow password access via Secret Service,
if you want to pass the password automatically,
you must enter it in an advanced field: C<ocs-utils-code>.
As for the URL and LOGIN, there is no need to worry, as they are retrieved via this protocol.

=head1 SEE ALSO

L<python3(1)>, L<playwright>, L<KeePassXC>

Source code: L<https://gricad-gitlab.univ-grenoble-alpes.fr/legi/soft/trokata/ocs-webutils/>


=head1 VERSION

OCS-Webutils version @VERSION@
