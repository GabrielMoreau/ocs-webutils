...
=head1 SYNOPSIS
 ...
 ocs-pkgpush[--help|-h] --url URL [--username USERNAME] [--password PASSWORD] --name NAME --description DESCRIPTION --priority PRIORITY --notif-text NOTIF_TEXT --notif-duration NOTIF_DURATION [--can-cancel NO] [--can-report YES] --file FILE [--headless] [--capture-dir CAPTURE_DIR]
...
Notification text shown to the end user. (Required)

=item B<--notif-duration> I<SECONDS>

Notification duration. (Required)
...
Push a package with a given priority and notification:
  ...
  ocs-pkgpush --url https://ocs.example.com/ocsreports \
    ...
    --priority 1 --notif-text "Update incoming" \
    --notif-duration 60 --file ./install.zip


=head1 SEE ALSO

L<python3(1)>, L<playwright>
